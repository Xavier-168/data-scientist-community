...
- 第三方许可以锁文件、已安装包元数据和上游官方许可页核实；原本地字体因缺少可追溯下载证据而不放行。候选仅新增从 Google Fonts 官方仓库取得、随附 OFL 1.1 与固定哈希的 Noto Serif SC 变量字体。
...
- 商业开发工作区新增维护者侧 `scripts/export_public_repo.py`：只复制白名单，不复制 `.git`；生产地址置空；社区访问、字体/资产白名单、Tauri 身份和原创应用图标以断言式变换生成。该脚本依赖私有输入树，按 manifest denylist 不进入公开候选；候选只公开输出清单、provenance 与验证工具。
...
- 原工作区 11 个来源无法证明的字体全部排除；候选仅分发可追溯到 Google Fonts 官方仓库、随附 OFL 1.1 的 Noto Serif SC 变量字体，其余位置继续使用系统 fallback。原应用图标全部排除；候选应用图标只含本次生成的原创社区占位 PNG 及其机械生成的多尺寸 ICNS。新增平台 SVG 仅用于数据来源识别，第三方名称和标识权利归对应平台；产品信箱 SVG 为本项目界面资产。
...
- 纯 Python staging 成功，未含 `.so`、`.pyc`、Nuitka、DMG、runtime、未批准字体或私有状态；允许的 Noto Serif SC 与 OFL 文件由白名单固定哈希。最终候选当前树扫描为 0 项。导出物不含 `.git`，发布者建立提交后仍须再做历史扫描。
